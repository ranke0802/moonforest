...
public sealed class PlaygroundView:MonoBehaviour {
 public WitchPlayer player;public PracticeDummy[] dummies;public const int SettingsVersion=415;public float yaw=144.1f,pitch=29.3f,distance=14,focusHeight=.6f,followSpeed=10,rotationSensitivity=3,zoomStep=.28f;Camera cam;Vector3 focus;float shakeUntil,shakeSize;
 ...
 [Serializable]public class Settings {public float yaw=144.1f,pitch=29.3f,distance=14,zoom=6.004f,height=.6f,follow=10,sensitivity=3,zoomStep=.28f,fov=48;public bool orthographic=true;}
 ...
 void Start(){ResetPracticeCamera();if(!RpgProgress.Instance.TestSession&&PlayerPrefs.GetInt("WitchRPG.CameraVersion",0)>=SettingsVersion){yaw=PlayerPrefs.GetFloat("WitchRPG.CameraYaw",144.1f);pitch=PlayerPrefs.GetFloat("WitchRPG.CameraPitch",29.3f);cam.orthographicSize=PlayerPrefs.GetFloat("WitchRPG.CameraZoom",6.004f);}}
 public void ResetPracticeCamera(){var d=new Settings();if(RpgProgress.Instance&&!RpgProgress.Instance.TestSession&&PlayerPrefs.HasKey("WitchRPG.BackView.v415"))d=JsonUtility.FromJson<Settings>(PlayerPrefs.GetString("WitchRPG.BackView.v415"));Apply(d);Snap();}
 public void FactoryCamera(){Apply(new Settings());Snap();}
 public void SaveBackView(){if(!RpgProgress.Instance.TestSession){PlayerPrefs.SetString("WitchRPG.BackView.v415",JsonUtility.ToJson(Capture()));PlayerPrefs.Save();}}
