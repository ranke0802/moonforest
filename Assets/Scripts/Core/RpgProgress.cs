...
[DefaultExecutionOrder(-80)]public sealed class RpgProgress:MonoBehaviour {
 ...
 public int Level{get;private set;}=1;public int XP{get;private set;}public int NeededXP=>30+(Level-1)*25;
 ...
 public void RestoreMana(){Mana=MaxMana;}public void AddMana(float amount){Mana=Mathf.Min(MaxMana,Mana+Mathf.Max(0,amount));}public void SetManaForTest(float value){Mana=Mathf.Clamp(value,0,MaxMana);}
 ...
 public void GainXP(int amount){if(amount<=0)return;int before=Level;XP=(int)Math.Min(100000000L,(long)XP+amount);RpgUI.Toast("+"+amount+" "+Loc.T("경험치","XP"));while(XP>=NeededXP&&Level<10000){XP-=NeededXP;Level++;PendingChoices++;ApplyStats(false);}if(Level>before)ForestSound.Play("level_up");if(PendingChoices>0&&Offers==null)BuildOffers();Save();}
 ...
 [Serializable]class Data {public int version;public LevelOffer[] offers;public int level,xp,attack,haste,vitality,pending,selected;public int[] ranks;public bool equipped;}
 public string Export()=>JsonUtility.ToJson(new Data{version=2,offers=Offers,level=Level,xp=XP,attack=attackTraining,haste=hasteTraining,vitality=vitalityTraining,pending=PendingChoices,ranks=ranks,equipped=Equipped,selected=(int)SelectedSkill});
 public void Save(){if(RpgSession.Instance)RpgSession.Instance.Save();}
 public void Import(string json){if(string.IsNullOrEmpty(json))return;var d=JsonUtility.FromJson<Data>(json);if(d.ranks==null||d.ranks.Length!=4)throw new Exception("Invalid save");Level=Mathf.Max(1,d.level);XP=d.version<2?Mathf.FloorToInt(Mathf.Clamp01(d.xp/(float)(40+(Level-1)*20))*NeededXP):Mathf.Max(0,d.xp);ranks=d.ranks;ranks[0]=Mathf.Max(1,ranks[0]);attackTraining=d.attack;hasteTraining=d.haste;vitalityTraining=d.vitality;Equipped=d.equipped;PendingChoices=d.pending;SelectedSkill=(SpellId)Mathf.Clamp(d.selected,1,3);Offers=d.offers!=null&&d.offers.Length==3?d.offers:null;if(PendingChoices>0&&Offers==null)BuildOffers();ApplyStats(true);ApplyEquipmentVisual();}
