...
[RequireComponent(typeof(CharacterController))]public sealed class SlimeMonster:MonoBehaviour {
 ...
 public int XPReward=>isBoss?(persistentDefeat?180:250):isElite?80:isBee?30:20;public string DisplayName=>isElite?Loc.T("정예 꿀날개","Elite Honeywing","精鋭の蜂"):isBoss?Loc.T("은빛 달늑대","Silver Moonwolf","銀月の狼"):isBee?Loc.T("꿀날개 벌","Honeywing Bee"):Loc.T("젤리 슬라임","Jelly Slime");public Vector3 HitPoint=>transform.position+Vector3.up*(isBoss?1.2f:isBee?.85f:.45f);
 ...
 void Update(){
  ...
  bool chasing=!TestIdle&&player&&!player.IsDead&&distance<(alerted?20:aggroRange)&&Vector3.ProjectOnPlane(home-player.transform.position,Vector3.up).magnitude<(alerted?24:leashRange);
  Vector3 dir=Vector3.zero;if(chasing){State="Chasing";dir=toward.normalized;if(distance<(isBee?1.05f:.85f)){dir=Vector3.zero;if(Time.time>=nextHit&&Time.time>=stunUntil&&!Frozen){ForestSound.Play(isBee?"bee_attack":"slime_attack",transform.position,.55f);bool critical=Random.value<.05f;player.TakeDamage(Mathf.RoundToInt((isElite?20:isBee?14:12)*(critical?1.5f:1)),critical,this);nextHit=Time.time+1.3f;}}}else{State="Idle";Vector3 back=Vector3.ProjectOnPlane(home-transform.position,Vector3.up);if(back.magnitude>.35f){dir=back.normalized;State="Returning";}}
