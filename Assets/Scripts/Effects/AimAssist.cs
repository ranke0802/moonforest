...
[DefaultExecutionOrder(90)] public sealed class AimAssist:MonoBehaviour {
 ...
 void LateUpdate(){if(Time.unscaledTime>=next){next=Time.unscaledTime+.1f;Target=null;if(!player.IsDead&&!RpgUI.Blocking&&player.Combat.ClassId!=1&&(!player.IsCharging||!player.ChargeUsesMouse))Target=player.Combat.ClassId==0?player.WitchAutoTarget():player.Combat.AimedBowTarget;}
