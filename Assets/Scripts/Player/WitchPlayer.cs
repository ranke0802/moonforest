...
public sealed class WitchPlayer:MonoBehaviour {
 ...
 public bool ReleaseCharge(Vector3 target){
  ...
  LastShotDirection=dir;var hit=stats?stats.EnergyHit(power,dir):new HitData{damage=DamageForCharge(power),owner=this,charge=power,knockback=2,direction=dir};if(AdventureProgress.Instance)hit=AdventureProgress.Instance.Relics.BasicHit(hit);LastDamage=hit.damage;var roster=GetComponent<HeroRoster>();if(roster&&roster.Selected==1)ElementalSpells.Melee(this,dir,hit);else {RpgVFX.Fire(origin+dir*.38f,dir,hit);if(roster&&roster.Selected==2){foreach(var bolt in FindObjectsByType<MagicProjectile>())if(bolt.owner==this&&Vector3.Distance(bolt.transform.position,origin)<1)bolt.arrow=true;}}ChangeState(roster&&roster.Selected>0?"Attack":"Cast",true);ForestSound.Play(power>.65f?"energy_heavy":"energy_light",origin,.8f);return true;
 }
