...
public sealed class WitchPlayer:MonoBehaviour {
 ...
 void Update(){
  ...
  Vector3 beforeMove=transform.position;var flags=motor.Move(((Combat.MovementOverride?Combat.Velocity:dir*(running?runSpeed:walkSpeed)*(IsCharging&&Combat.ClassId!=2?.55f:1))+Vector3.up*verticalSpeed+impactVelocity)*Time.deltaTime);impactVelocity=Vector3.Lerp(impactVelocity,Vector3.zero,1-Mathf.Exp(-10*Time.deltaTime));movedSpeed=Vector3.ProjectOnPlane(transform.position-beforeMove,Vector3.up).magnitude/Mathf.Max(Time.deltaTime,.001f);if(animator){animator.speed=1;animator.SetFloat("MoveRate",State=="Walk"?Mathf.Clamp(movedSpeed/walkStrideSpeed,.1f,2.7f):State=="Run"?Mathf.Clamp(movedSpeed/runStrideSpeed,.1f,2.4f):1);}
 ...
 public bool ReleaseCharge(Vector3 target){
  ...
  Vector3 origin=transform.position+Vector3.up*.85f;if(Combat.ClassId==0&&!ChargeUsesMouse){var enemy=WitchAutoTarget();target=enemy?enemy.HitPoint:origin+transform.forward*30;}
  ...
  LastShotDirection=dir;var hit=stats?stats.EnergyHit(power,dir):new HitData{damage=DamageForCharge(power),owner=this,charge=power,knockback=2,direction=dir};LastDamage=hit.damage;var roster=GetComponent<HeroRoster>();if(roster&&roster.Selected==1)ElementalSpells.Melee(this,dir,hit);else {RpgVFX.Fire(origin+dir*.38f,dir,hit);if(roster&&roster.Selected==2){foreach(var bolt in FindObjectsByType<MagicProjectile>())if(bolt.owner==this&&Vector3.Distance(bolt.transform.position,origin)<1)bolt.arrow=true;}}ChangeState(roster&&roster.Selected>0?"Attack":"Cast",true);ForestSound.Play(power>.65f?"energy_heavy":"energy_light",origin,.8f);return true;
 }
