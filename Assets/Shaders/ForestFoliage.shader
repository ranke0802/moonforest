Shader "Forest/Readable Foliage" {
 ...
 void surf(Input IN,inout SurfaceOutputStandard o){
  ...
  if(_AdventureView.w>0){float3 va=_AdventureView.xyz+float3(0,1,0);float3 vv=b-va;float vt=saturate(dot(IN.worldPos-va,vv)/max(dot(vv,vv),.001));if(vt>.02&&vt<.98)clip(distance(IN.worldPos,va+vt*vv)-3.8);}
